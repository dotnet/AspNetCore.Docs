Installing ASP.NET Core On Windows
===============================

By `Rick Anderson`_, `Steve Smith`_, `Daniel Roth`_

This page shows you how to install ASP.NET Core on Windows. To run ASP.NET core apps on IIS, see :doc:`/publishing/iis`.

.. contents:: In this article:
  :local:
  :depth: 1

Install ASP.NET Core with Visual Studio
------------------------------------

The easiest way to get started building applications with ASP.NET core is to install the latest version of Visual Studio 2015 (including the free Community edition). 
...
2. Install `ASP.NET core <https://go.microsoft.com/fwlink/?LinkId=627627>`_. 
  
  This will install the latest ASP.NET core runtime and tooling.
  
3. Enable the ASP.NET core command-line tools. Open a command-prompt and run::
...
You are all set up and ready to write :doc:`your first ASP.NET core application </tutorials/your-first-aspnet-application>`!

Install ASP.NET core from the command-line
---------------------------------------

You can also install ASP.NET core from the command-line. There are a few steps involved, since we'll need to install and configure the environment in which ASP.NET runs, the :doc:`/dnx/index`. To install DNX, we need one more tool, the .NET Version Manager (DNVM).
