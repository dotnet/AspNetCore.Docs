...
Publishing an ASP.NET Core 1.0 project to an IIS server with Web Deploy requires a few
...
instructions to get started with publishing an ASP.NET Core 1.0 web application using
Web Deploy to any IIS host.

To publish an ASP.NET Core 1.0 application to a remote IIS server the following steps
are required.

1. Configure your remote IIS server to support ASP.NET Core 1.0
...
The HttpPlatformHandler is a new component that connects IIS with your ASP.NET Core 1.0
...
`Provisioning PowerShell script <https://github.com/aspnet/DataProtection/blob/dev/Provision-AutoGenKeys.ps1>`_ for each application pool you will be hosting ASP.NET Core 1.0 applications under.
...
You can find more info on configuring your IIS server for ASP.NET Core 1.0 at
...
an ASP.NET Core 1.0 application to a standard IIS host is to use a publish profile. If
