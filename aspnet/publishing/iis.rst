...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
1. Create an ASP.NET Core 1.0 app. In this sample, I'll create an MVC 6 app using the **Web Site** template under **ASP.NET Core 1.0 Preview Templates**.
...
- Set the application pool to **No Managed Code**. ASP.NET Core 1.0 runs in a separate process and manages the runtime.
