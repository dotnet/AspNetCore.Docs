...
[!code[Main](owin-middleware-in-the-iis-integrated-pipeline/samples/sample1.xml)]
...
[!code[Main](owin-middleware-in-the-iis-integrated-pipeline/samples/sample2.xml?highlight=10)]
...
2. From the Package Manager Console (PMC), run the following command: 
    ...
    [!code[Main](owin-middleware-in-the-iis-integrated-pipeline/samples/sample3.xml)]
3. Add an `OWIN Startup Class` and name it `Startup`. Replace the generated code with the following (the changes are highlighted):  
    ...
    [!code[Main](owin-middleware-in-the-iis-integrated-pipeline/samples/sample4.xml?highlight=5-7,15-36)]
...
[!code[Main](owin-middleware-in-the-iis-integrated-pipeline/samples/sample5.xml)]
...
[!code[Main](owin-middleware-in-the-iis-integrated-pipeline/samples/sample6.xml?highlight=13,19)]
...
[!code[Main](owin-middleware-in-the-iis-integrated-pipeline/samples/sample7.xml)]

## Stage Marker Rules

Owin middleware components (OMC) can be configured to run at the following OWIN pipeline stage events:

[!code[Main](owin-middleware-in-the-iis-integrated-pipeline/samples/sample8.xml)]
...
3. The OWIN pipeline and the IIS pipeline is ordered, therefore calls to `app.UseStageMarker` must be in order. You cannot set the event handler to an event that precedes the last event registered with to `app.UseStageMarker`. For example, *after* calling:
    ...
    [!code[Main](owin-middleware-in-the-iis-integrated-pipeline/samples/sample9.xml)]
...
4. The earliest stage of calls to `app.UseStageMarker` wins. For example, if you switch the order of `app.UseStageMarker` calls from our previous example:
    ...
    [!code[Main](owin-middleware-in-the-iis-integrated-pipeline/samples/sample10.xml?highlight=13,19)]

 The output window will display: 

    [!code[Main](owin-middleware-in-the-iis-integrated-pipeline/samples/sample11.xml)]
