...
.. Note:: There can be at most one parameter decorated with [FromBody]. This is because MVC delegates the responsibility of reading the request stream to the formatter and once the request stream is read for a parameter, it might not be possible to read the request stream again for binding other [FromBody] parameter(s) as the stream has been consumed already. 
