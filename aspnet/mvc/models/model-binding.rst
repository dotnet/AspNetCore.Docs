...
In order for binding to happen, members of classes must be public, writable properties containing a default public constructor. Private fields are not bound. Properties of type ``IEnumerable`` or array must be settable and will be populated with an array. Collection properties of type ``ICollection<T>`` can be read-only. 
