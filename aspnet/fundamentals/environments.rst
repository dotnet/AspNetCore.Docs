...
The current environment setting can be detected programmatically from within ASP.NET 5. In addition, ASP.NET MVC 6 introduces an `Environment Tag Helper <https://docs.asp.net/projects/mvc/en/latest/views/tag-helpers/index.html>`_ that allows MVC Views to include certain sections based on the current application environment.
