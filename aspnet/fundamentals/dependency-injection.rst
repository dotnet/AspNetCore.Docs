...
.. tip:: You can request certain framework-provided services within ``Startup`` methods via their parameter lists - see :doc:`startup` for more details.
...
.. note:: Keep in mind, in addition to these two types, everything else in ``ConfigureServices`` is also adding and configuring services. For example, ``services.AddMvc()`` adds the services MVC requires.
...
.. warning:: Prefer requesting dependencies as constructor parameters to accessing the ``RequestServices`` collection. 
