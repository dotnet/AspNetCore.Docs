...
This article describes how to configure an Internet Information Services (IIS) web server to support web publishing and deployment with the IIS Web Deploy Handler.
...
- **Web Deploy Remote Agent Service** 
     ...
     This approach requires less web server configuration, but you need to provide local server administrator credentials to deploy anything to the server.

 - **Web Deploy Handler**

     This approach is more complex and requires more initial effort to set up the web server. However, with this approach, you can configure IIS to allow non-administrator users to perform deployment. The Web Deploy Handler is only available in IIS version 7 or later.

 - **Offline deployment**

    This approach requires the least web server configuration of the web server, but a server administrator must manually copy the web package onto the server and import it through IIS Manager.
...
Yes, if you want to allow non-administrator users to deploy content to specific IIS websites. This approach can be desirable in these scenarios:
...
This section guides you through installing the required products and components on the web server. Before you begin, a good practice is to run Windows Update to ensure that your server is fully up to date.

In this case, you need to install the following components:
...
5. In the **Microsoft .NET Framework 4** row, if the .NET Framework is not already installed, click **Add**.
    ...
    > [!NOTE]
    > You may have already installed the .NET Framework 4.0 through Windows Update. If a product or component is already installed, the Web Platform Installer repace indicate this by replacing the **Add** button with the text **Installed**.
