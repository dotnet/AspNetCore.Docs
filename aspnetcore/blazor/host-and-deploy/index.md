...
The best location within `<head>` markup ([location of `<head>` content](xref:blazor/project-structure#location-of-head-and-body-content)) for placement of the `<base>` tag is immediately after the opening `<head>` tag. This ensures that the following elements with attribute values that are URLs, such as the `href` attributes of `<link>` elements, receive the relative root path from the `<base>` tag.
