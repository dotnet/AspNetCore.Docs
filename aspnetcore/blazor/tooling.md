...
:::moniker range=">= aspnetcore-8.0"
  ...
  > [!NOTE]
  > The hosted Blazor WebAssmebly project template isn't available from an ASP.NET Core 8.0 or later shared framework. To create a hosted Blazor WebAssembly app, a **Framework** option earlier than .NET 8.0 must be selected with the **ASP.NET Core Hosted** checkbox.
...
* For a hosted Blazor WebAssembly experience with demonstration code and Bootstrap, add the hosted option (`-ho`/`--hosted`) to the command:
  ...
  > [!NOTE]
  > The hosted Blazor WebAssmebly project template isn't available from an ASP.NET Core 8.0 or later shared framework. To create a hosted Blazor WebAssembly app using a .NET 8.0 or later SDK, pass the `-f|--framework` option with a 7.0 target framework (`net7.0`):
  >
  > ```dotnet cli
...
* For a hosted Blazor WebAssembly experience, add the hosted option (`-ho` or `--hosted`) option to the command:
  ...
  > [!NOTE]
  > The hosted Blazor WebAssmebly project template isn't available from an ASP.NET Core 8.0 or later shared framework. To create a hosted Blazor WebAssembly app using a .NET 8.0 or later SDK, pass the `-f|--framework` option with the target framework moniker (for example, `net6.0`):
  >
  > ```dotnet cli
