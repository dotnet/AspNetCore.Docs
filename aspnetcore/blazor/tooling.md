...
[WebAssembly Single Instruction, Multiple Data (SIMD)](https://aka.ms/dotnet-wasm-features#simd---single-instruction-multiple-data) can improve the throughput of vectorized computations by performing an operation on multiple pieces of data in parallel using a single instruction. SIMD is enabled by default.
