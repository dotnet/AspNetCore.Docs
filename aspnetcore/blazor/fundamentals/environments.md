...
For a standalone Blazor WebAssembly app running locally, the development server adds the `Blazor-Environment` header with the value it obtains from the `ASPNETCORE_ENVIRONMENT` environment variable, which is usually set by default to "`Development`" in the project's `Properties/launchSettings.json` file.
