...
## Alternative approach with Azure Files
...
Consider an approach that uses Azure File Shares, which offers the following benefits:

* Upload files from the client directly to Azure Files with the [Azure Storage File Share client library](/javascript/api/overview/azure/storage-file-share-readme) or the [Azure Files REST API](/rest/api/storageservices/file-service-rest-api).
* Authorize user uploads with a [user-delegated shared-access signature (SAS) token](/javascript/api/overview/azure/storage-file-share-readme?view=azure-node-latest#with-sas-token) generated by the app for each client file upload.
* Provide automatic redundancy and file share backup.
* Limit uploads with file share quotas.
