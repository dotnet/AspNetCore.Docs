...
The `::deep` combinator affects where the scope attribute is applied to the rule. When you define a CSS rule in a scoped CSS file, the scope is applied to the right most element by default. For example: `div > a` is transformed to `div > a[{STRING}]`, where the `{STRING}` placeholder is a ten-character string generated by the framework (for example, `b-3xxtam6d07`). If you instead want the rule to apply to a different selector, the `::deep` combinator allows you do so. For example, `div::deep > a` is transformed to `div[{STRING}] > a` (for example, `div[b-3xxtam6d07] > a`).
