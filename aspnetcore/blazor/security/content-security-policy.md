...
The `ImportMap` integrity value is computed at for each app request because the `ImportMap` component renders unique content each time the page is generated for fingerprinted assets (.NET 9 or later).
