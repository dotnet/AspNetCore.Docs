...
Blazor Identity depends on <xref:Microsoft.EntityFrameworkCore.DbContext> instances not created by a factory (<xref:Microsoft.EntityFrameworkCore.IDbContextFactory%601>, [Blazor documentation](xref:blazor/blazor-ef-core#new-dbcontext-instances)), which is intentional because <xref:Microsoft.EntityFrameworkCore.DbContext> is sufficient for the project template's Identity components that render statically and don't support interactivity.
