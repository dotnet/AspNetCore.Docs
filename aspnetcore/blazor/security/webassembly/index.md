...
The authentication support in Blazor WebAssembly (`Authentication.js` on the client) is built on top of the [Microsoft Authentication Library (MSAL, `msal.js`)](/azure/active-directory/develop/msal-overview), which is used to handle the underlying authentication protocol details.
