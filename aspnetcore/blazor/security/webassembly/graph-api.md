...
The examples in this article that configure MSAL with scopes provisioned for Microsoft Graph use <xref:Microsoft.Authentication.WebAssembly.Msal.Models.MsalProviderOptions.DefaultAccessTokenScopes%2A> to provision the scopes, not <xref:Microsoft.Authentication.WebAssembly.Msal.Models.MsalProviderOptions.AdditionalScopesToConsent%2A>. 

A new app user signing-in to an app for the first time is only able to provision Graph API permission with `User.Read` scope if the scope is provisioned for MSAL with <xref:Microsoft.Authentication.WebAssembly.Msal.Models.MsalProviderOptions.DefaultAccessTokenScopes%2A>.

If the app passes `User.Read` scope to MSAL configuration with <xref:Microsoft.Authentication.WebAssembly.Msal.Models.MsalProviderOptions.AdditionalScopesToConsent%2A>, a new user can sign-in to the app for the first time with default `openid`, `profile`, and `offline_access` provisioned scopes for their access token, but accessing Graph API for user data after sign-in with `await Client.Me.GetAsync()` results in an exception stating that an access token for Graph API isn't present:

> :::no-loc text="Microsoft.Graph.Models.ODataErrors.ODataError: Access token is empty.":::

After a user provisions Graph scopes with scopes configured for MSAL via <xref:Microsoft.Authentication.WebAssembly.Msal.Models.MsalProviderOptions.DefaultAccessTokenScopes%2A>, an app can then configure MSAL user consent to use <xref:Microsoft.Authentication.WebAssembly.Msal.Models.MsalProviderOptions.AdditionalScopesToConsent%2A> for subsequent user sign-ins. However, changing app code makes no sense for a production app that requires the periodic addition of new users with delegated Graph permissions.
...
<!-- UPDATE 8.0 I had to remove the following link because the build
    ...
    Trying to add the link back to the list now.
-->
