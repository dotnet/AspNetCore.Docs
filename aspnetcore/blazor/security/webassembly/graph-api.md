...
<xref:Microsoft.Authentication.WebAssembly.Msal.Models.MsalProviderOptions.AdditionalScopesToConsent%2A> isn't used because it's unable to provision Graph API scopes for users when they sign-in to the app for the first time with MSAL via the Azure consent UI. When the user attempts to access Graph API for the first time, they're confronted with an exception:
