...
> [!NOTE]
> Pass the App ID URI to the `app-id-uri` option, but note that a configuration change might be required, which is described in the [Access token scopes](#access-token-scopes) section.
