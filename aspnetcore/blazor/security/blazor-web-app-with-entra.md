...
* `{KEY IDENTIFIER}`: Azure Key Vault key identifier used for key encryption. The key vault name is `contoso`, and an access policy allows the application to access the key vault with `Get`, `Wrap Key`, and `Unwrap Key` permissions. The key name is `data-protection`. The version of the key for the `{KEY VERSION}` placeholder is obtained from the key in the Entra or Azure Portal after it's created.
