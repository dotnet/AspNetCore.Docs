...
* Selecting the first button, which calls `HandleSelect1`, triggers a rerender.
* Selecting the second button, which calls `HandleSelect2`, doesn't trigger a rerender.
* Selecting the third button, which calls `HandleSelect3`, doesn't trigger a rerender and uses [event arguments](xref:blazor/components/event-handling#event-arguments) (<xref:Microsoft.AspNetCore.Components.Web.MouseEventArgs>).
