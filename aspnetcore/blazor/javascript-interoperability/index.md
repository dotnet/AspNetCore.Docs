...
Don't assume that observing `document.body`, instead of `target.parentNode`, is a more reliable target. Observing `document.body` has performance implications because callback logic is executed for *all* DOM updates, whether or not they have anything to do with your element. Use either of the following approaches:

* In cases where you can identify a suitable ancestor node to observe, use `MutationObserver` with it. Ideally, this ancestor is scoped to the changes you want to observe, rather than `document.body`.
