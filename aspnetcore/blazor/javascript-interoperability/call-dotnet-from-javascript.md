...
To avoid a memory leak and allow garbage collection, the .NET object reference created by <xref:Microsoft.JSInterop.DotNetObjectReference> is disposed when the object referece goes out of scope when the method returns with [`using var` syntax](/dotnet/csharp/language-reference/keywords/using-statement).
