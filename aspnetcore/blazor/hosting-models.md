...
1. In the app's layout file (*_Layout.cshtml*):
   ...
   * Add the following `<base>` tag to the `<head>` element:
     ...
     The `href` value (the *app base path*) in the preceding example assumes that the app's components are routable at the app's root path. For more information on setting the app base path, see <xref:host-and-deploy/blazor/index#app-base-path>.
