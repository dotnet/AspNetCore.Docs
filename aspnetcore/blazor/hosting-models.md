...
The [.NET Intermediate Language (IL) interpreter](/dotnet/standard/managed-code#intermediate-language--execution) includes partial [just-in-time (JIT) runtime](/dotnet/standard/glossary#jit) support to achieve improved runtime performance. The JIT interpreter optimizes execution of interpreter bytecodes by replacing them with tiny blobs of WebAssembly code. The JIT interpreter is automatically enabled for Blazor WebAssembly apps.
