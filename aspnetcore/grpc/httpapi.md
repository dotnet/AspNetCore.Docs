...
There is *experimental* support for generating OpenAPI from gRPC transcoded RESTful APIs. The NuGet package integrates with [Swashbuckle](xref:tutorials/get-started-with-swashbuckle) and is experimental in .NET 7 to give us time to explore the best way to provide OpenAPI support.
