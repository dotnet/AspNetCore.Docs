...
By default a `<Protobuf>` reference will generate both a concrete client and a service base class. `GrpcServices` attribute can be used to limit C# asset generation to either the concrete client or service base class. Valid `GrpcServices` are:

* `Server`
* `Client`
* `Both` (also the default)
