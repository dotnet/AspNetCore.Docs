...
gRPC ASP.NET Core has built-in support for gRPC health checks with the [`Grpc.AspNetCore.HealthChecks`](https://www.nuget.org/packages/Grpc.AspNetCore.HealthChecks) package. Results from .NET health checks are reported to gRPC health checks callers. To configure gRPC health checks in an app:
...
* .NET health checks registered with the app are periodically executed for health results.
...
By default, the gRPC health checks service uses all registered health checks to determine health status. To run a subset of health checks, provide a predicate with `MapService` when registering health check services with `AddGrpcHealthChecks`.

```csharp

```

gRPC health checks supports checking individual components by specifying a service name. By default, all health results from .NET health checks are aggregated together and reported

gRPC health checks integrates and the .NET health check infrastructure using the `Grpc.AspNetCore.HealthChecks` package. Results from .NET health checks are reported to gRPC health checks callers.

To configure `Grpc.AspNetCore.HealthChecks`:

1. Add a package reference to `Grpc.AspNetCore.HealthChecks`.
2. 
