...
| DisposeHttpClient | `false` | If set to `true` and an `HttpMessageHandler` or `HttpClient` is specified, the `HttpClient` is disposed when the `GrpcChannel` is disposed. |
