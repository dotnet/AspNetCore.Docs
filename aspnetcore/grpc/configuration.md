...
| `Interceptors` | None | A collection of interceptors that are run with each gRPC call. Interceptors are run in the order they are registered, and globally configured interceptors are run before interceptors configured for a single service. See [gRPC Interceptors vs Middleware](xref:grpc/migration#grpc-interceptors-vs-middleware) for more information about gRPC interceptors. |
