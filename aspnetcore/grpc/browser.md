...
There are pros and cons to each approach. If you are already using Envoy as a proxy in your app's environment then it might make sense for you to also use it to provide gRPC-Web support. On the other hand, if you want a simple solution for gRPC-Web that only requires ASP.NET Core then `Grpc.AspNetCore.Web` is a good choice.
