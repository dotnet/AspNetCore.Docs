...
> [!NOTE]
> The gRPC client types are generated automatically by the build process. The tooling package [Grpc.Tools](https://www.nuget.org/packages/Grpc.Tools/) generates the following files based on the *test.proto* file in the sample code:
>
> * `Common\obj\Debug\[TARGET_FRAMEWORK]\netstandard2.0\Test.cs`: The protocol buffer code which populates, serializes and retrieves the request and response message types.
> * `Common\obj\Debug\[TARGET_FRAMEWORK]\netstandard2.0\TestGrpc.cs`: Contains the the generated client classes.
>
