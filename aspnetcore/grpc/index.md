...
Services, clients and messages are automatically generated by referencing the [Grpc.Tools](https://www.nuget.org/packages/Grpc.Tools/) package and *\*.proto* files in a project:
...
`GreeterService` inherits from the generated `GreeterBase` type. It is made accessible to clients in *Startup.cs*.
...
A gRPC client is created using a channel, which is configured to call services using the server address.
