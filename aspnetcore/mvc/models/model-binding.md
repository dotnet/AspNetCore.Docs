...
In order for binding to happen the class must have a public default constructor and the members to be bound must be public writable properties. When model binding happens the class will only be instantiated using the public default constructor, then the properties can be set.
