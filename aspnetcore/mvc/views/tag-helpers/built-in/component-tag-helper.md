...
The preceding example assumes that the `Counter` component is in the app's *Pages* folder. The placeholder `{APP ASSEMBLY}` is the app's namespace (for example, `@using BlazorSample.Pages`).
...
The preceding example assumes that the `ColorfulCheckbox` component is in the app's *Shared* folder. The placeholder `{APP ASSEMBLY}` is the app's namespace (for example, `@using BlazorSample.Shared`).
...
The preceding example assumes that the `MyComponent` component is in the app's *Shared* folder. The placeholder `{APP ASSEMBLY}` is the app's namespace (for example, `@using BlazorSample` and `@using BlazorSample.Shared`). `MyClass` is in the app's namespace.
