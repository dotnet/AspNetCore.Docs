...
In the preceding code, the Script Tag Helper generated the second script ( `<script>  (window.jQuery || document.write(`) element, which tests for `window.jQuery`. If `window.jQuery` is found, the primary link is used; if not, the HTML is updated with the fallback link.
