...
Explicit expressions can be used to render output from generic methods in *.cshtml* files. The following markup shows how to correct the error caused by brackets in a generic from the implicit section. The code is written as an explicit expression:
