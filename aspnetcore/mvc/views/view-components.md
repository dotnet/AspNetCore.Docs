...
When considering if view components meet an app's specifications, consider using Blazor components instead. Blazor components also combine markup with C# code to produce reusable UI units. Blazor components are designed for developer productivity when providing client-side UI logic and composition. For more information, see <xref:blazor/components/index>. For information on how to incorporate Blazor components into an MVC or Razor Pages app, see <xref:blazor/components/prerendering-and-integration?pivots=server>.
