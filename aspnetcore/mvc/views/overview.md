...
Views are typically returned from actions as a [`ViewResult`](ViewResult). Your action method can create and return a `ViewResult` directly, but more commonly if your controller inherits from [`Controller`](Controller), you'll simply use the `View` helper method, as this example demonstrates:
...
> [!NOTE]
> You can customize the default convention regarding where views are located within the app by using a custom [`IViewLocationExpander`](IViewLocationExpander).
