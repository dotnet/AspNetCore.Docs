...
Logging configuration is commonly provided by the `Logging` section of *appsettings*.`{Environment}`*.json* files. The following example shows the contents of *appsettings.Development.json* generated by the ASP.NET Core web app templates:
...
For information on configuring the logging providers by other file formats, the command line, environment variables, and more, see <xref:fundamentals/configuration/index>.
