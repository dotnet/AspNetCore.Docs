...
When running on IIS and IIS Express, a *502.5 Process Failure* is returned by IIS and IIS Express if the process can't be started. Follow the troubleshooting advice in the [Troubleshoot ASP.NET Core on IIS](xref:host-and-deploy/iis/troubleshoot) topic.
