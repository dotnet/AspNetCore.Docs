...
Once session is installed and configured, you refer to it via HttpContext, which exposes a property called `Session` of type [`ISession`](ISession). You can use this interface to get and set values in `Session`, such as `byte[]`.
