...
The OpenAPI endpoint doesn't enable any authorization checks by default. However, it's possible to limit access to the OpenAPI document to those with the `tester` scope by using the following configuration:
