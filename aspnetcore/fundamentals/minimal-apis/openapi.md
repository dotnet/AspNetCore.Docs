...
The [OpenAPI specification](https://spec.openapis.org/oas/latest.html)) is a programming language-agnostic standard for documenting HTTP APIs. This standard is supported in minimal APIs
through a combination of built-in APIs and open-source libraries. There are three key aspects to OpenAPI integration in your application:

* Generating information about the endpoints in your application
...
Minimal APIs provides built-in support for generating information about endpoint in an application via the `Microsoft.AspNetCore.OpenApi` package. For exposing the generated OpenAPI
definition via a visual UI, you will need to integrate a third-party dependency. 
