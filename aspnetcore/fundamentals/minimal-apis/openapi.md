...
* `UseSwagger`adds the [Swagger middleware](xref:tutorials/get-started-with-swashbuckle#add-and-configure-swagger-middleware). You need `Swashbuckle.AspNetCore` nuget package to use this command.
* `UseSwaggerUI` enables an embedded version of the Swagger UI tool in development mode.
