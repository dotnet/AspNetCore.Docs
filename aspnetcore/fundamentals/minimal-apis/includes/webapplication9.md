...
## Web application builder (`WebApplication`)
...
The preceding samples can use the environment variables `ASPNETCORE_HTTP_PORTS` and `ASPNETCORE_HTTPS_PORTS` to set ports if `ASPNETCORE_URLS` isn't used:

* :::no-loc text="ASPNETCORE_HTTP_PORTS=3000;5005":::
* :::no-loc text="ASPNETCORE_HTTPS_PORTS=5000":::
