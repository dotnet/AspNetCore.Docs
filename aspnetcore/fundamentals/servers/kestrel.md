...
While this is a good trade off in most cases there are times where an application may want to terminate the request immediately after writing the response. For example, the clients may have restrictive data caps and limiting uploaded data might be a priority. In those cases, the application may call `HttpContext.Abort()` from either a controller or a custom middleware. However, there are caveats to this. Besides it being expensive to close and open tcp connections, there is no guarantee that the client has actually read the response  before the connection closes. Calling `HttpResponse.CompleteAsync()` before calling `HttpContext.Abort()` will at least ensure that the response has left the server, but client behavior is still not completely predictable.
