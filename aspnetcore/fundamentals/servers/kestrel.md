...
While this is a good trade off in most cases there are times where an application may want to terminate the request immediately after writing the response. For example, the clients may have restrictive data caps and limiting uploaded data might be a priority. In those cases, the application may call `HttpContext.Abort()` from either a controller or a custom middleware. However, there are caveats to this. Besides it being expensive to close and open tcp connections, there is no assurance that the client has actually read the response  before the connection closes. Calling `HttpResponse.CompleteAsync()` before calling `HttpContext.Abort()` will at least ensure that the response has left the server, but client behavior is still not completely predictable.

This approach should therefore be used only when a specific problem needs to be solved, such as if rouge clients are trying to post garbage data or when there is a bug in client code that causes large or numerous requests. If possible, it's better for clients to utilize the 100 continue http response code, so that they wait for the server to respond before starting to send the request body.
