...
| `Http1AndHttp2AndHttp3` | HTTP/1.1, HTTP/2 and HTTP/3. The first client request normally uses HTTP/1.1 or HTTP/2, and the [`alt-svc` response header prompts the client to upgrade to HTTP/3](xref:fundamentals/servers/kestrel/http3#alt-svc). HTTP/2 and HTTP/3 requires TLS; otherwise, the connection defaults to HTTP/1.1. |
