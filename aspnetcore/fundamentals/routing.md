...
In addition to the built-in route constraints, custom route constraints can be created by implementing the <xref:Microsoft.AspNetCore.Routing.IRouteConstraint> interface. The `IRouteConstraint` interface contains a single method, <xref:Microsoft.AspNetCore.Routing.IRouteConstraint.Match>, which should return true if the constraint is satisfied and false otherwise.
