...
Routing uses *routes* (implementations of [`IRouter`](IRouter)) to:
...
Routing is connected to the [middleware](middleware.md) pipeline by the [`RouterMiddleware`](RouterMiddleware) class. [ASP.NET MVC](../mvc/overview.md) adds routing to the middleware pipeline as part of its configuration. To learn about using routing as a standalone component, see [using-routing-middleware](#using-routing-middleware).
...
Incoming requests enter the [`RouterMiddleware`](RouterMiddleware) which calls the [`RouteAsync`](RouteAsync) method on each route in sequence. The [`IRouter`](IRouter) instance chooses whether to *handle* the request by setting the [`RouteContext`](RouteContext) [`Handler`](Handler) to a non-null
[`RequestDelegate`](RequestDelegate). If a handler is set a route, it will be invoked to process the request and no further routes will be processed. If all routes are executed, and no handler is found for a request, the middleware calls *next* and the next middleware in the request pipeline is invoked.

The primary input to `RouteAsync` is the [`RouteContext`](RouteContext) [`HttpContext`](HttpContext) associated with the current request. The `RouteContext.Handler` and [`RouteContext`](RouteContext) [`RouteData`](RouteData) are outputs that will be set after a successful match.
...
[`RouteData`](RouteData) [`Values`](Values) is a dictionary of *route values* produced from the route. These values are usually determined by tokenizing the URL, and can be used to accept user input, or to make further dispatching decisions inside the application.

[`RouteData`](RouteData) [`DataTokens`](DataTokens)  is a property bag of additional data related to the matched route. `DataTokens` are provided to support associating state data with each route so the application can make decisions later based on which route matched. These values are developer-defined and do **not** affect the behavior of routing in any way. Additionally, values stashed in data tokens can be of any type, in contrast to route values which must be easily convertable to and from strings.

[`RouteData`](RouteData) [`Routers`](Routers) is a list of the routes that took part in successfully matching the request. Routes can be nested inside one another, and the `Routers` property reflects the path through the logical tree of routes that resulted in a match. Generally the first item in `Routers` is the route collection, and should be used for URL generation. The last item in `Routers` is the route that matched.
...
URL generation follows a similar iterative process, but starts with user or framework code calling into the [`GetVirtualPath`](GetVirtualPath) method of the route collection. Each *route* will then have its `GetVirtualPath` method called in sequence until a non-null [`VirtualPathData`](VirtualPathData) is returned.

The primary inputs to `GetVirtualPath` are:

* [`VirtualPathContext`](VirtualPathContext) [`HttpContext`](HttpContext)

* [`VirtualPathContext`](VirtualPathContext) [`Values`](Values)

* [`VirtualPathContext`](VirtualPathContext) [`AmbientValues`](AmbientValues)
...
The output of `GetVirtualPath` is a [`VirtualPathData`](VirtualPathData). `VirtualPathData` is a parallel of `RouteData`; it contains the `VirtualPath` for the output URL as well as the some additional properties that should be set by the route.

The [`VirtualPathData`](VirtualPathData) [`VirtualPath`](VirtualPath) property contains the *virtual path* produced by the route. Depending on your needs you may need to process the path further. For instance, if you want to render the generated URL in HTML you need to prepend the base path of the application.

The [`VirtualPathData`](VirtualPathData) [`Router`](Router) is a reference to the route that successfully generated the URL.

The [`VirtualPathData`](VirtualPathData) [`DataTokens`](DataTokens) properties is a dictionary of additional data related to the route that generated the URL. This is the parallel of `RouteData.DataTokens`.

### Creating routes

Routing provides the [`Route`](Route) class as the standard implementation of `IRouter`. `Route` uses the *route template* syntax to define patterns that will match against the URL path when [`RouteAsync`](RouteAsync) is called. `Route` will use the same route template to generate a URL when [`GetVirtualPath`](GetVirtualPath) is called.

Most applications will create routes by calling `MapRoute` or one of the similar extension methods defined on [`IRouteBuilder`](IRouteBuilder). All of these methods will create an instance of `Route` and add it to the route collection.

> [!NOTE]
> [`MapRoute`](MapRoute) doesn't take a route handler parameter - it only adds routes that will be handled by the [`DefaultHandler`](DefaultHandler). Since the default handler is an [`IRouter`](IRouter), it may decide not to handle the request. For example, ASP.NET MVC is typically configured as a default handler that only handles requests that match an available controller and action. To learn more about routing to MVC, see [ð"§ Routing to Controller Actions](../mvc/controllers/routing.md).
...
>[!TIP]
> An app using ASP.NET MVC should use [`UrlHelper`](UrlHelper) to generate URLs instead of calling into routing directly.
...
* [`Build`](Build)
* [`MapGet`](MapGet)  Matches only HTTP GET requests
* [`UseRouter`](UseRouter)
...
Some of these methods such as `MapGet` require a [`RequestDelegate`](RequestDelegate) to be provided. The `RequestDelegate` will be used as the *route handler* when the route matches. Other methods in this family allow configuring a middleware pipeline which will be used as the route handler. If the *Map* method doesn't accept a handler, such as `MapRoute`, then it will use the [`DefaultHandler`](DefaultHandler).
...
The second parameter to the [`VirtualPathContext`](VirtualPathContext) constructor is a collection of *ambient values*. Ambient values provide convenience by limiting the number of values a developer must specify within a certain request context. The current route values of the current request are considered ambient values for link generation. For example, in an ASP.NET MVC app if you are in the `About` action of the `HomeController`, you don't need to specify the controller route value to link to the `Index` action (the ambient value of `Home` will be used).
