...
Configuration Providers that implement change detection have the ability to reload configuration when an underlying settings is changed (after app startup). For example, The File Configuration Provider (described later in this topic) and the [Azure Key Vault Configuration Provider](xref:security/key-vault-configuration) implement change detection.
