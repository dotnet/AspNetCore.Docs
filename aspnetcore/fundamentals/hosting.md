...
ASP.NET Core apps require a *host* in which to execute. A host must implement the [`IWebHost`](IWebHost) interface, which exposes collections of features and services, and a `Start` method. The host is typically created using an instance of a [`WebHostBuilder`](WebHostBuilder), which builds and returns a  [`WebHost`](WebHost) instance. The `WebHost` references the server that will handle requests. Learn more about [servers](servers.md).
...
The `WebHostBuilder` is responsible for creating the host that will bootstrap the server for the app. `WebHostBuilder` requires you provide a server that implements [`IServer`](IServer) (`UseKestrel` in the code above). `UseKestrel` specifies the Kestrel server will be used by the app.
