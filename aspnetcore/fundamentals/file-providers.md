...
File Providers are an abstraction over file systems. The main interface is [IFileProvider](IFileProvider). `IFileProvider` exposes methods to get file information ([IFileInfo](IFileInfo)), directory information ([IDirectoryContents](IDirectoryContents)), and to set up change notifications (using an [IChangeToken](IChangeToken)).
