...
## Register sets of services with extension methods

The ASP.NET Core framework uses a convention for registering sets of related services. The convention is to use a single `Add{FEATURE_NAME}` extension method to register all of the services required by a framework feature. For example, the <Microsoft.Extensions.DependencyInjection.MvcServiceCollectionExtensions.AddControllers> extension method registers the services required for MVC controllers.
