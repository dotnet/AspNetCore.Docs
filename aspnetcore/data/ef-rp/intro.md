...
Note: The preceding code uses `Models` for the namespace (`namespace ContosoUniversity.Models`) rather than `Data`. `Models` is used to be consistent with the scaffolder generated code. For more information, see [this Github scaffolding issue](https://github.com/aspnet/Scaffolding/issues/822).
