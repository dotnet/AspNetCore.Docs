...
When editing an instructor record, you may want to update the instructor's office assignment. The Instructor entity has a one-to-zero-or-one relationship with the OfficeAssignment entity, therefore the code must handle:

* If the user clears the office assignment that had a value, delete the `OfficeAssignment` entity.
...
-  Gets the current `Instructor` entity from the database using eager loading for the `OfficeAssignment` navigation property. T

-  Updates the retrieved `Instructor` entity with values from the model binder. The `TryUpdateModel` overload enables you to whitelist the properties. `TryUpdateModel` prevents [overposting](xref:data/ef-rp/crud#overposting).
...
[!code-cshtml[Main](intro/samples/cu/Pages/Instructors/Edit.cshtml?highlight=32-36)]
...
A check boxes enables changes to courses an instructor is assigned to. A check box for every course in the database is displayed. Courses that the instructor is assigned to are checked. The user can select or clear check boxes to change course assignments. If the number of courses were much greater:

* You'd probably use a different user interace to display the classes.
...
A view model class is used to contain data for the list of check boxes.

Create *SchoolViewModels\AssignedCourseData.cs* with the following code:

[!code-csharp[Main](intro/samples/cu/Models/SchoolViewModels/AssignedCourseData.cs)]

[!code-csharp[Main](intro/samples/cu/Pages/Instructors/Edit.cshtml.cs?name=snippet&highlight=20-23,39-)]
