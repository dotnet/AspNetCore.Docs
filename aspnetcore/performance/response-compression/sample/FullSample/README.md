...
2. Add an `Accept-Encoding: gzip` header and note the compressed response size and response headers. You will see the response size drop and the `Content-Encoding: gzip` response header. When you look at the response body for the Lorem Ipsum text or **testfile1kb.txt** file, you will see that the text has been compressed and is unreadable. Note the dramaitc decreate in payload size for **banner.svg** when the `Accept-Encoding: gzip` header is added to the request for that file. 
