...
* Collation information is included to make APIs such as [StringComparison.InvariantCultureIgnoreCase](xref:System.StringComparison.InvariantCultureIgnoreCase) work correctly. If you can verify that the app doesn't require the collation data, consider disabling it by setting the `BlazorWebAssemblyPreserveCollationData` MSBuild property in the app's project file to `false`:
