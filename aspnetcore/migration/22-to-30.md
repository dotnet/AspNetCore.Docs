...
## Namespace change for AddAuthorization methods

The ASP.NET Core 2.2 and lower `AddAuthorization` methods in `Microsoft.AspNetCore.Authorization`:

* Have  been renamed `AddAuthorizationCore`.
* Have been moved to `Microsoft.AspNetCore.Authorization.Policy`.

Apps that are using both `Microsoft.AspNetCore.Authorization` and `Microsoft.AspNetCore.Authorization.Policy` are not impacted.

Apps that are not using `Microsoft.AspNetCore.Authorization.Policy` should do one of the following:
