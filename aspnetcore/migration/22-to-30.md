...
* Prior to ASP.NET Core 3.0, the default UI framework was Bootstrap 3. The UI framework could be set to Bootstrap 4 with the following code:
  ...
  In ASP.NET Core 3.0, <xref:Microsoft.AspNetCore.Identity.IdentityBuilderUIExtensions.AddDefaultUI*> no longer provides an overload for configuring the UI framework used by Identity UI. The default UI framework is now Bootstrap 4:
