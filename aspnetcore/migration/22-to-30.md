...
There is a new option for controlling message size limits from clients.
...
Before, you could set the `TransportMaxBufferSize` and that would effectively control the max message size. In 3.0 that option now only controls the max size before backpressure is observed.
