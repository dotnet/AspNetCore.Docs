...
## Opt in to runtime compilation of Razor views

In ASP.NET Core 2.2, support for runtime compilation of Razor views was included out-of-the-box when calling `AddMvc`. This allowed changes to be made to views at runtime and for those changes to be seen by refreshing the browser without needing to restart the app. It also enabled Razor views that didn't exist when the app was built to to be generated at runtime. The following code shows all that was needed before ASP.NET Core 3.0:
...
In ASP.NET Core 3.0, runtime compilation of Razor views is an opt-in scenario. To enable the same support as found in ASP.NET Core 2.2, add a reference to the [Microsoft.AspNetCore.Mvc.Razor.RuntimeCompilation](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Razor.RuntimeCompilation/) NuGet package and call `AddRazorRuntimeCompilation`:
...
For more information, see <xref:mvc/views/view-compilation>.
