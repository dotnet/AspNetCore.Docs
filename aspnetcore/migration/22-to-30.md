...
To use Json.NET in an ASP.NET Core 3.0 SignalR project, see [Switch to Newtonsoft.Json](xref:#switch-to-newtonsoftjson) in this document.
...
Libraries may often need to support multiple versions of ASP.NET Core. Most libraries that were compiled against previous versions of ASP.NET Core should continue working without issues. However if your library relies on a feature that has a binary [breaking change](<xref:#breaking-changes>) or wants to take advantage of new features in ASP.NET Core 3.0, the recommendation is to cross-compile the application. For exammple,
