...
* If the app isn't already using Razor Pages, add a call to `MapRazorPages`. See [Razor Pages](#razor-pages) in this document.
* Prior to ASP.NET Core 3.0, the default UI framework was Bootstrap 3. The UI framework could be set to Bootstrap 4 with the following code:
  ...
  In ASP.NET Core 3.0, <xref:Microsoft.AspNetCore.Identity.IdentityBuilderUIExtensions.AddDefaultUI*> no longer provides an overload for configuring the UI framework used by Identity UI. The default UI framework his now Bootstrap 4:

  ```csharp
  services.AddIdentity<IdentityUser, IdentityRole>()
      .AddDefaultUI() // uses Bootstrap 4

  // or

  services.AddDefaultIdentity<IdentityUser>() // includes a call to AddDefaultUI()
  ```

  To configure the UI framework used by Identity UI in ASP.NET Core 3.0, see [this announcement](https://github.com/dotnet/docs/issues/14959).
