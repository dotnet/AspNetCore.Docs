...
## Blazor event handlers and bind attributes

In ASP.NET Core Preview 9 or later, the set of bindings and event handlers available for HTML elements are:

* Defined in the `Microsoft.AspNetCore.Components.Web` assembly (`Microsoft.AspNetCore.Components.Web` NuGet package).
* Found in the `Microsoft.AspNetCore.Components.Web` namespace.

### Migration

**Blazor WebAssembly and Razor Class Libraries**

When migrating an app to ASP.NET Core Preview 9 or later:

1. Add a package reference to the `Microsoft.AspNetCore.Components.Web` package in the project file.
1. Open the app's root *_Imports.razor* file and add `@using Microsoft.AspNetCore.Components.Web`.

**Blazor server-side**

When migrating an app to ASP.NET Core Preview 9 or later, open the app's root *_Imports.razor* file and add `@using Microsoft.AspNetCore.Components.Web`.

> [!NOTE]
> A Blazor server-side app receives a `Microsoft.AspNetCore.Components.Web` package reference implicitly, so manually adding the package reference to the app's project file isn't required.

### Troubleshoot

If `@bind` or `@on...` attributes are colorized as a normal HTML attributes (shown in the following image) in Visual Studio, the `@using` statement is missing:

![Index.razor component CSHTML markup in Visual Studio showing HTML colorized unemphasized font for bind and onclick attributes.](https://user-images.githubusercontent.com/1430011/64197856-f1679300-ce3b-11e9-83b7-71ffa6d68fec.png)

With the correct package reference and `@using` statement, event handlers and data binding attributes, such as `@onclick` and `@bind`, appear in a bold font and colorized as Razor syntax as shown in the following images when using Visual Studio:

![Index.razor component CSHTML markup in Visual Studio showing Razor colorized bold font for bind and onclick attributes.](https://user-images.githubusercontent.com/1430011/64197727-a3528f80-ce3b-11e9-8cc4-fe34a858eb49.png)
...
Try adding `@using Microsoft.AspNetCore.Components.Web` to the file. If you now see an error like the following, the reference to the `Microsoft.AspNetCore.Components.Web` package is missing from the project.
...
If adding the `@using` statement fixes the problem, consider moving the statement to the app's root *_Imports.razor* so that it applies to all files.
