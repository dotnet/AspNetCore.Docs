...
Console apps creating framework dependent builds that use a package that depends on the ASP.NET Core [shared framework](https://natemcmaster.com/blog/2018/08/29/netcore-primitives-2/) may have the following build error:
...
The ASP.NET Core shared framework is contained in `Microsoft.AspNetCore.App`. To determine the app has a dependency on the ASP.NET Core shared framework, examine the *runtimeconfig.json* file. The following JSON file shows a dependency on the ASP.NET Core shared framework:
