...
The following error appears if the reference to the `Microsoft.AspNetCore.Components.Web` package is missing from the project:
...
**The @using statement isn't applied to all project files**

If the `@using` statement isn't applied to all files, the following error appears:
...
Consider moving the `@using` statement to the app's root *_Imports.razor*.
