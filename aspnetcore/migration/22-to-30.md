...
Use `AddJsonProtocol` to set serializer options.
...
If you were previously using `AddJsonProtocol` with Newtonsoft.Json options or want to continue using Newtonsoft.Json instead of the new default the following instructions will show you how:

1. Install the Microsoft.AspNetCore.SignalR.Protocols.NewtonsoftJson NuGet package.
1. On the client add `.AddNewtonsoftJsonProtocol()` to the `HubConnectionBuilder`:
```csharp
new HubConnectionBuilder()
    .WithUrl("/chatHub")
    ...
    .Build();
```
1. On the server add `.AddNewtonsoftJsonProtocol()` to the `AddSignalR()` call:
```csharp
services.AddSignalR()
    .AddNewtonsoftJsonProtocol(...);
```
