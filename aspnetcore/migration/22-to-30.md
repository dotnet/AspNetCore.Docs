...
Authorization can also be configured for specific classes of endpoints. The following code is an example of converting an MVC app that configured a global `AuthorizeFilter` to an app with a specific policy to use `RequireAuthorization`:

[!code-csharp[](22-to-30/samples/Startup.cs?name=snippet&highlight=8-9,21-22,26-27,53-54)]

TODO - use snippet to update the following:

Policies can also be customized. The `DefaultPolicy` is configured to require authentication and a specific scope:

[!code-csharp[](22-to-30/samples/Startup.cs?name=snippet2&highlight=21-26,52)]
