...
Alternatively, endpoint requests processed by authorization middleware can require authorization without `[Authorize]` or `RequireAuthorization()` by configuring a fallback policy. The default policy is selected by `[Authorize]` or `RequireAuthorization()` when no policy name is specified. The fallback policy is selected when no policy is produced from authorization metadata. The fallback policy is `null` by default, so authorization isn't required in that case. For complete policy selection rules, see <xref:security/authorization/policies#default-and-fallback-policies>.

The following example is the same as the preceding default policy example but uses a fallback policy to require authentication for endpoint requests except when `[AllowAnonymous]` is specified:
