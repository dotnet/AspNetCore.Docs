...
The `UseServer` is no longer available for [`IWebHostBuilder`](IWebHostBuilder). You must now use [`UseKestrel`](UseKestrel) or [`UseWebListener`](UseWebListener).

## ASP.NET MVC Core

The `HtmlEncodedString` class has been replaced by [`HtmlString`](HtmlString) (contained in the  `Microsoft.AspNetCore.Html.Abstractions` package).

## Security

The [`AuthorizationHandler<TRequirement>`](AuthorizationHandler<TRequirement>) class now only contains an asynchronous interface.
