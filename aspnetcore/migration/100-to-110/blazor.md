...
Use the following instructions to adopt the new JS module approach for the navigation bar toggler in an existing app.
...
Also in the app's `NavMenu` component, change the line that has the inline JS to toggler the navigation bar:
...
If the app has a [Content Security Policy (CSP)](xref:blazor/security/content-security-policy#server-side-blazor-apps) with an unsafe hash for the inline JS, remove the unsafe hash:
