...
## Update CORS policy if when credentials are allowed with wildcard origin

As of ASP.NET Core 2.2, the CORS middleware responds with a wildcard origin (`*`) if a policy is configured to allow any origin and allow credentials. Credentials are not supported when a wildcard origin (`*`) is specified and browsers will disallow the CORS request. See https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS/Errors/CORSNotSupportingCredentials for more details on this along with  ways to correct this problem on the client.

To correct this problem on the server, either

a) modify the CORS policy to no longer allow credentials, that is remove the call to xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowCredentials* when configuring the policy, 

b) or, if credentials are required for the CORS request to succeed, modify the policy to specify allowed hosts e.g. `builder.WithOrigins("https://api.example1.com", "https://example2.com")` instead of using xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowAnyOrigin*
