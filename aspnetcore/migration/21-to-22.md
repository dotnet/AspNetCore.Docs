...
In this article, we walk you through updating an existing ASP.NET Core 2.1 project to ASP.NET Core 2.2.
...
Projects targeting .NET Core should use the [TFM](/dotnet/standard/frameworks#referring-to-frameworks) of a version greater than or equal to .NET Core 2.2. Search for the `<TargetFramework>` node in the project file, and replace its inner text with `netcoreapp2.2`:
...
If your solution relies upon a [global.json](/dotnet/core/tools/global-json) file to target a specific .NET Core SDK version, update its `version` property to use the 2.2 version installed on your machine:
