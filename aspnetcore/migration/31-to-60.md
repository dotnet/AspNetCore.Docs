...
This article explains how to update an existing ASP.NET Core 3.1 project to ASP.NET Core in .NET 6. To upgrade from ASP.NET Core 5.0 to 6.0, see <xref:migration/50-to-60>.
...
Code samples migrated to the new minimal hosting model in ASP.NET Core in .NET 6](xref:migration/50-to-60-samples) and [Migrate from ASP.NET Core 5.0 to 6.0](xref:migration/50-to-60)
...
For apps using Docker, update your *Dockerfile* `FROM` statements and scripts. Use a base image that includes the ASP.NET Core in .NET 6 runtime. Consider the following `docker pull` command difference between ASP.NET Core 3.1 and 6.0:
