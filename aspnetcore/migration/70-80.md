...
1. Follow the guidance in the first three sections of this article:
   ...
   * [Update the .NET SDK version in `global.json`](#update-the-net-core-sdk-version-in-globaljson)
...
1. Move the file content from the `Client/wwwroot/index.html` file to a new `App` component file (`App.razor`) created at the root of the `Server` project.
...
1. Make the following changes to the `App.razor` file:
   ...
   Add the `HeadOutlet` component to the `<head>` content with the interactive WebAssembly render mode and prerendering disabled:

   ```razor
   <HeadOutlet @rendermode="new InteractiveWebAssemblyRenderMode(prerender: false)" />

   Replace the app `div` tag in `App.razor` with the `Routes` component. Start by removing the `div` tag:
   ...
   Add the `Routes` component with the interactive WebAssembly render mode and prerendering disabled:
...
1. Update `Server/Program.cs`:
   ...
   Replace the preceding line with a call to `MapRazorComponents`, supplying the `App` component as the root component type, and add a chained call to `AddInteractiveWebAssemblyRenderMode`:
