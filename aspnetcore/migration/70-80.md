...
1. Also in the `.Client` project file, remove the [`Microsoft.AspNetCore.Components.WebAssembly.DevServer`](https://www.nuget.org/packages/Microsoft.AspNetCore.Components.WebAssembly.DevServer) package reference:
...
1. Also in the `.Client` project, add an entry to the `_Imports.razor` file to make shorthand render modes available to the app:
...
1. Make the following changes to the `App.razor` file:
   ...
   Replace the pair of `<title>` tags with a `HeadOutlet` component. Start by removing the `<title>` tag. Note the title for use later before removing the tag: 

   ```diff
   - <title>...</title>
   ```

   Replacement `HeadOutlet` component at the end of the `<head>` content with the Interactive WebAssembly render mode (prerendering disabled):
   ...
   * `{SERVER PROJECT ASSEMBLY NAME}`: Server project assembly name. Example: `BlazorSample.Server`
   ...
   Open the `.Client` project's layout file (`Shared/MainLayout.razor`) and add a `PageTitle` component with the app's default page title, using content of the `<title>` tag earlier for the `{TITLE}` placeholder:
   ...
   > [!NOTE]
   > Other layout files should also receive the `PageTitle` component with default website title.
   >
