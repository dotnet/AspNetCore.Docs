...
1. Move the file content from the `Client/wwwroot/index.html` file to a new `Host.razor` file created at the root of the `Client` project.
...
1. Update `Server/Program.cs`:
   ...
   Add a `using` statement for `` at the top of the file.
...
1. To enable prerendering:
   ...
   Update all components to support prerendering from the server.

   <!-- What's in the sample is highly template-specific and won't match what they
        need to do generally. We need to work out what to tell devs for this step. -->
