...
1. Update `Program.cs`:
   ...
   * Replace `AddServerSideBlazor` with `AddRazorComponents` and a chained call to `AddServerComponents`:
   ...
   * Replace `MapBlazorHub` with `MapRazorComponents`, supplying the `App` component as the root component type, and a chained call to `AddServerRenderMode`.
