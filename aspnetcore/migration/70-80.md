...
1. Make the following changes to the `App.razor` file:
   ...
   Replace the website's default title (`<title>...</title>`) with a `HeadOutlet` component. Start by removing the title tags and title. Note the title for use later:
   ...
   * `{SERVER PROJECT ASSEMBLY NAME}`: Server project assembly name. Example: `BlazorSample.Server`
   ...
   Open the `.Client` project's layout file (`.Client/Shared/MainLayout.razor`) and add a `PageTitle` component with the app's default page title, using content of the `<title>` tag earlier for the `{TITLE}` placeholder:
