...
1. Update the SDK and the following properties:
...
1. In .NET 5, Blazor WebAssembly relies on the .NET SDK to provide IL trimming (previously refered to as linker). Trimming is performed on "Publish" regardless of the application's build configuration. Listed below are replacements to trimmer options previously available as Blazor-specific MSBuild settings:

  - `BlazorWebAssemblyEnableLinking` is replaced by `PublishTrimmed` MSBuild property:
  ...
  - `BlazorLinkerDescriptor` item group is replaced by `TrimmerRootDescriptor`
  - `AdditionalMonoLinkerOptions` previously available to configure the linker is no longer available. Additional settings to configure the trimmer are documented in <xref:dotnet/core/deploying/trim-self-contained />
  - `BlazorWebAssemblyI18NAssemblies` and `BlazorWebAssemblyPreserveCollationData` properties previously supported by the trimmer are no longer required in .NET 5.0 apps.
