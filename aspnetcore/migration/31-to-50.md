...
1. In `Shared/MainLayout.razor`, surround the content in a `<div>` element with an `id` of `page`:
...
1. The latest base `wwwroot/css/app.css` files includes the following styles. Remove extra styles that have migrated to other files leaving the following styles and any that you've added for your own styling.

    `wwwroot/css/app.css` (base styles not including any added by the developer):
