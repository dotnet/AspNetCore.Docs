...
* The compilation relied on an individual `RazorGenerate` and `RazorCompile` target to produce the generated code. These targets are not valid in .NET 6. In .NET 6, both code generation and compilation are supported by a single call to the compiler.
