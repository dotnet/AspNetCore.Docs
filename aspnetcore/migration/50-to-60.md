...
* Unifies *Startup.cs* and *Program.cs* into a smaller *Program.cs* file.
...
* <xref:Microsoft.AspNetCore.Hosting.StartupBase.Configure*> is replaced with configuration calls to .
...
No. The new hosting model is the preferred way to host ASP.NET Core apps using .NET 6 and later, but you aren't forced to change your project layout. This means apps can upgrade from .NET 5 to .NET 6.0 by changing the target framework in the project file from `net5.0` to `net6.0`. See [Update the target framework](#tf) in this document. <!-- REVIEW REQUIRED --> We recommend apps migrate to the new hosting model to take advantage of new features only available to the new hosting model.
