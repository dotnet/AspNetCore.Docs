...
* For 2.1, we recommend using [Razor Class Library](xref:razor-pages/ui-class) as the way to distribute Razor files. However, if your application uses embedded views, or otherwise relies on runtime compilation of Razor files, add `<CopyRefAssembliesToPublishDirectory>true</CopyRefAssembliesToPublishDirectory>` to your project file.
