...
* For 2.1, a [Razor Class Library](xref:razor-pages/ui-class) is the recommended solution to distribute Razor files. If your app uses embedded views, or otherwise relies on runtime compilation of Razor files, add `<CopyRefAssembliesToPublishDirectory>true</CopyRefAssembliesToPublishDirectory>` to your project file.
...
## Changes to launchSettings file

As ASP.NET Core apps now use https by default, the `Properties/launchSettings.json` file has changed.

The following JSON shows the template-generated 2.0 launchSettings.json file:

[!code-json[Main](20_21/sample/launchSettings20.json)]

The following JSON shows the template-generated 2.1 launchSettings.json file:

[!code-json[Main](20_21/sample/launchSettings21.json)]

See see [Enforce HTTPS in ASP.NET Core](xref:security/enforcing-ssl) for more information.
