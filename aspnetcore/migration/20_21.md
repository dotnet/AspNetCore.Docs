...
Identity 2.1 exposes all endpoints with the `Identity` area. For example, the follow table shows examples of Identity endpoints that change from 2.0 to 2.1:
...
* Delete the *Pages/Account/* folder.
...
* [Scaffold Identity](xref:security/authentication/scaffold-identity) into your project:
  ...
  * Select **Add** to create a new Data context class. Creating a new data context is required for scaffold. You remove the new data context in the next section.
...
[!code-csharp[Main](20_21/sample/Startup.cs?name=snippet)]
