...
In this model, the parameter and it's property are both named `Search`. This would result in model binding attempting to bind the property as `search.Search`. This might be undesirable if the intent is to be able to bind to a value without a prefix such as a query that looks like `?search=MySearchTerm`.

## How to fix violations

* Rename the parameter if it's prefix is not used during binding:
```csharp
...
```csharp
public IActionResult Get([Bind(Prefix = "")] SearchModel model)
{
    ...
}
```
