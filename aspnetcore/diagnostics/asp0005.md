...
When an endpoint is declared, attributes should be applied on the delegate parameter in order to be effective. For example, the `Authroize` attribute in the following code sample will not be set on the registered endpoint.
...
Instead, the attribute should be placed on the route handler parameter as follows.
