...
No diagnostic is reported when the configure action passed to `AddAuthorization` uses one of the following members of `AuthorizationOptions`:
...
`AuthorizationBuilder` doesn't have equivalents for these members, so they can't be converted.

No diagnostic is reported if the configure action passed to `AddAuthorization` contains operations unrelated to `AuthorizationOptions`. Unrelated operations could cause unintentional deletion of code when applying the code fix. It would not be easy and might not be possible to automatically map unrelated operations to the fluent API of `AddAuthorizationBuilder`.

The following example is code that triggers this diagnostic:
...
The following example shows the result of the code fix:
...
This diagnostic is information only. Suppress warnings if you don't want to use the new syntax.
