...
> *  Use `await` when calling asynchronous methods that depend on the connection staying open. For example, a method such as `Clients.All.SendAsync(...)` can fail if it is called without `await` , and the hub method completes before `SendAsync` finishes.
