...
Update the `/etc/nginx/sites-available/default` file to redirect HTTP requests to HTTPS. The following example redirects requests for `example.com`/`*.example.com`:
