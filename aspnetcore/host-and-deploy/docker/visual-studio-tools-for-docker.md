...
If you want different behavior based on the build configuration (for example, Debug or Release), you can provide that by adding configuration-specific *docker-compose* files. The filenames should be named according to the build configuration (for example, *docker-compose.vs.debug.yml* and *docker-compose.vs.release.yml*) and placed in the same location as the *docker-compose-override.yml* file. 

![Docker compose override files](visual-studio-tools-for-docker/_static/docker-compose-overrides.png)
