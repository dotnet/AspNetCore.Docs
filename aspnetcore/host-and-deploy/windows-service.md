...
1. In the project file:
   ...
   1. Confirm the presence of the runtime identifier or add it to the **\<PropertyGroup>** that contains the target framework:
      ...
      If mulitple Runtime Identifiers (RIDs) are listed in a semicolon-delimited list, use the property name `<RuntimeIdentifiers>` (plural). For more information, see [.NET Core RID Catalog](/dotnet/core/rid-catalog).
