...
When clients download uncompressed Blazor files, the MIME type might be incorrect for an app published from the default Blazor WebAssembly template and based on your approach to uploading the app to Azure Storage. For example, the Azure Storage Storage Explorer or a third-party tool, such as [MSP360™ Microsoft Azure Storage Explorer for Windows](https://www.msp360.com/explorer/windows/microsoft-azure.aspx), may assign the correct MIME type in the `Content-Type` header for some files but not others. After deploying the app, confirm or update the appropriate MIME type or `Content-Type` header value for the uncompressed app files, which are the non-Gzip (*.gz*) and non-Brotli (*.br*) files:
