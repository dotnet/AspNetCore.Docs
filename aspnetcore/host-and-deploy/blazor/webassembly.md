...
Uncompressed Blazor files in the deployment must have the correct the MIME type when clients download them from Azure Storage. [Microsoft Azure Storage Storage Explorer](/vs-azure-tools-storage-manage-with-storage-explorer) or a third-party tool, such as [MSP360™ Microsoft Azure Storage Explorer for Windows](https://www.msp360.com/explorer/windows/microsoft-azure.aspx), may assign the correct MIME type in the `Content-Type` header for some files but not others by default. Your tooling may allow you to configure the MIME type for the `Content-Type` header from a file's extension. If a tool doesn't allow you to configure the MIME type by extension, update any incorrect MIME types after deployment. The uncompressed app files that may require attention are the files that don't have a Gzip (*.gz*) or Brotli (*.br*) file extension:
