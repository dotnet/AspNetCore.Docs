...
When clients download uncompressed Blazor files, the MIME type might be incorrect for an app's files published from the default Blazor WebAssembly template and based on your approach to uploading the app to Azure Storage. For example, [Microsoft Azure Storage Storage Explorer](/vs-azure-tools-storage-manage-with-storage-explorer) or a third-party tool, such as [MSP360™ Microsoft Azure Storage Explorer for Windows](https://www.msp360.com/explorer/windows/microsoft-azure.aspx), may assign the correct MIME type in the `Content-Type` header for some files but not others. Either deploy the app with the correct MIME types in the `Content-Type` headers or update any incorrect MIME types after deployment. The uncompressed app files are the files that don't have a Gzip (*.gz*) or Brotli (*.br*) file extension:
