...
1. The following parameters are available when running the installer from an administrator command shell:
   ...
   * `OPT_NO_SHARED_CONFIG_CHECK=1`: Disable the check for using an IIS Shared Configuration when the shared configuration (`applicationHost.config`) is on the same machine as the IIS installation. *Only available for ASP.NET Core 2.2 or later Hosting Bundler installers.* For more information, see [ASP.NET Core 3.1 Module](https://docs.microsoft.com/en-us/aspnet/core/host-and-deploy/aspnet-core-module?view=aspnetcore-3.1#aspnet-core-module-with-an-iis-shared-configuration), [ASP.NET Core 5 Module](https://docs.microsoft.com/en-us/aspnet/core/host-and-deploy/iis/advanced?view=aspnetcore-5.0#aspnet-core-module-with-an-iis-shared-configuration)
