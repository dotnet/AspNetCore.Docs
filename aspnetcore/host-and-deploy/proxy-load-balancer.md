...
To forward the scheme from the proxy in non-IIS scenarios, configure the Forwarded Headers Middleware by setting the `ASPNETCORE_FORWARDEDHEADERS_ENABLED` to `true`. Enabling the Forwarded Headers Middleware is designed for cloud environments and it doesn't enable features such as the [`KnownProxies option`](#forwarded-headers-middleware-options) to restrict which IPs forwarders are accepted from.
