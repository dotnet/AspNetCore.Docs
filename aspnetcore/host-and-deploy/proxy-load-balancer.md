...
To forward the scheme from the proxy in non-IIS scenarios, configure the [Forwarded Headers Middleware](xref:core/host-and-deploy/proxy-load-balancer#forwarded-headers-middleware-options) by setting the `ASPNETCORE_FORWARDEDHEADERS_ENABLED` environment variable has been `true`. Setting setting the `ASPNETCORE_FORWARDEDHEADERS_ENABLED =true` is designed for cloud environments and it doesn't enable features such as the [`KnownProxies option`](xref:core/host-and-deploy/proxy-load-balancer#forwarded-headers-middleware-options) to restrict which IPs forwarders are accepted from.
