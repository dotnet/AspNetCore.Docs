...
| `requestTimeout` | <div class="NOTE alert"><p>Note</p><p class="x-hidden-focus">ASP.NET Core 2.1 is in preview and not recommended for production use.</p></div><p>Optional timespan attribute.</p><p>Specifies the duration for which the ASP.NET Core Module waits for a response from the process listening on %ASPNETCORE_PORT%.</p><p>In versions of the ASP.NET Core Module that shipped with the release of ASP.NET Core 2.1 or later, the `requestTimeout` is specified in hours, minutes, and seconds.</p> | `00:02:00` |
::: moniker-end
