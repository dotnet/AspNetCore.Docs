...
Notes: If you receive an error with the Install-Package command, Quick Launch (Ctrl+Q) NuGet Package Manager and search for this pacakge (Microsoft.EntityFrameworkCore.Tools). This will also allow you to install the package or check if it is already installed. Alternatively, see the [CLI approach](#cli) if you have problems with the PMC.
