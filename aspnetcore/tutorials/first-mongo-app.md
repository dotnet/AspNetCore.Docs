...
1. Download and Install MongoDB Shell:
   * macOS/Linux: Choose a directory to extract the MongoDB Shell to. Add the resulting path for `mongosh.exe` to the `PATH` environment variable.
...
1. Choose a Data Storage Directory: Select a directory on your development machine for storing data. Create the directory if it doesn't exist. The mongo Shell doesn't create new directories:
   * macOS/Linux: For example, `C:\\BooksData`.
   * Windows: For example, `/usr/local/var/mongodb`.
