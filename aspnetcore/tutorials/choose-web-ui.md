...
There are benefits and drawbacks to consider for either approach. A hybrid approach is also possible.  For example, the majority of the web UI is rendered on the server, and client rendered pages are added as needed.

### When to choose a server rendered UI approach

A web UI app that renders on the server dynamically generates the page's HTML as well as CSS on the server. The page arrives at the client ready to display.
...
* Uses a token based security model, rather than using cookies which may or may not be enabled on the browser.
...
* Rich interactivity is not instant: Rich interactive-driven changes are best generated on the client, rather than requiring a round-trip to the server to generate an updated page.
...
* Supports incremental updates, saving partially completed forms or documents without the user having to click a button to submit a form.
* Can be designed to run in a disconnected mode with updates to a client-side model that are eventually synchronized back to the server once a connection is re-established.
...
The following section briefly explains the ASP.NET Core web UI server rendered models available and directs you to complete overviews, tutorials and app samples to get started.  ASP.NET Core Razor Pages and ASP.NET Core MVC are server-based frameworks for building web apps with .NET.
...
* Easier to scale the web UI application in terms of complexity. It is easier to code, debug, and test something (model, view, or controller) that has a single job.
...
The following section briefly explains the ASP.NET Core web UI client rendered models available and directs you to complete overviews, tutorials and app samples to get started.
...
* Reusable, sharable UI compenents: Blazor apps are composed of reusable web UI components implemented using C#, HTML, and CSS. Both client and server code is written in C#, allowing you to share code and libraries.
...
* Quickly get productive with Blazor re-usable UI components from top component vendors like:
...
ASP.NET Core Blazor Server provides a model for building rich, interactive, and composable user interfaces in C#, with the additional advantage of having direct access to server resources. Blazor Server components are UI elements such as a page, dialog or data entry form, that handle client-side web UI interactions.  The client-side web UI interactions are handled over a direct and continuous websocket connection and Blazor can span multiple connections to increase speed. Blazor components keep track of the state of the client-side DOM and effeciently apply updates to only the components that need it.
...
* Prerendering:
  * Executes Blazor components on the server and renders them into a page or view.
	* Improves the perceived load time of the app while interactivity is being setup.
...
### ASP.NET Core Single Page Application (SPA) with JavaScript Framworks such as Angular and React
...
Benefits of ASP.NET Core SPA with JavaScript Framworks, in addition to the client rendering benefits previously listed:
