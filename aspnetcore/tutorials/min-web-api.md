...
* On the Scalar page, select the **POST /todoitems** operation, then select **Test Request**.
...
The Scalar UI displays the response, including the status code and response body. The response body shows the `id` was set to `1`. A 201 `HTTP` status code is returned, indicating that the request was successfully processed and resulted in the creation of a new resource.
...
* Call **GET /todoitems/{id}** in Scalar to return data from a specific id:
  * Select **GET /todoitems/{id}**, then **Test Request**.
...
* Select **PUT /todoitems/{id}**, then **Test Request**.
...
* Select **PATCH /todoitems/{id}**, then **Test Request**.
...
Verify you can post and get the `Secret` field with the current endpoints.
