...
The `@page` Razor directive makes the file an MVC action, which means that it can handle requests. `@page` must be the first Razor directive on a page. `@page` and `@model` are examples of transitioning into Razor-specific markup. For more information, see [Razor syntax](xref:mvc/views/razor#razor-syntax).
...
[Layout](xref:mvc/views/layout) templates let you:
...
[!INCLUDE[](~/tutorials/razor-pages/page/includes/page9.md)]
