...
[Implicit cast operators](/dotnet/csharp/language-reference/keywords/implicit) in the `ActionResult<T>` API support the conversion of both `T` and `ActionResult` to `ActionResult<T>`. `T` converts to `ObjectResult`, which means `return T;` is equivalent to `return new ObjectResult(T);`.
