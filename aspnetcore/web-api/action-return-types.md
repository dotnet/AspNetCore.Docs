...
* A 201 return code is generated by the [CreatedAtAction](/dotnet/api/microsoft.aspnetcore.mvc.controllerbase.createdataction) helper method when a product is created. In this path, the `Product` object is returned.
* A 400 status code ([BadRequest](/dotnet/api/microsoft.aspnetcore.mvc.controllerbase.badrequest)) is returned by the ASP.NET Core runtime when model validation fails. For example, the following model indicates that requests must provide the `Name` property and a value. Therefore, failure to provide a proper `Name` in the request causes model validation to fail.
