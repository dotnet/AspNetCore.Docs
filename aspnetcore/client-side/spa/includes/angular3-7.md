...
When the proxy is launched, the target URL and port is inferred from the environment variables set by .NET, `ASPNETCORE_URLS`. To set the URLs or HTTPS port, use the environment variable or change the value in `proxy.conf.json`.
