...
2. Complete the **GET NEW ACCESS TOKEN** dialog as follows:
    ...
    | **Scope**                 | `https://<tenant domain name>/api/user_impersonation openid offline_access`                   | Replace *&lt;tenant domain name&gt;* with the tenant's domain name without angle brackets. *api* represents the name of the Web API project. You can also use Application ID. The pattern for the URL is: *https://{tenant}.onmicrosoft.com/{app_name_or_id}/{scope name}*.  |
