...
Policies are applied using the [`Policy`](Policy) property on the [`AuthorizeAttribute`](AuthorizeAttribute) attribute;
...
If you want to specify multiple allowed roles in a requirement then you can specify them as parameters to the [`RequireRole`](RequireRole) method;
