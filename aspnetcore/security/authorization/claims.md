...
You then apply the policy using the [`Policy`](Policy) property on the [`AuthorizeAttribute`](AuthorizeAttribute) attribute to specify the policy name;
...
The [`AuthorizeAttribute`](AuthorizeAttribute) attribute can be applied to an entire controller, in this instance only identities matching the policy will be allowed access to any Action on the controller.
...
If you have a controller that is protected by the [`AuthorizeAttribute`](AuthorizeAttribute) attribute, but want to allow anonymous access to particular actions you apply the [`AllowAnonymousAttribute`](AllowAnonymousAttribute) attribute;
