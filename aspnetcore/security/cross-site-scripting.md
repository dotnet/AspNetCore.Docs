...
Validation can be a useful tool in limiting XSS attacks. For example, a numeric string containing only the characters 0-9 won't trigger an XSS attack. Validation becomes more complicated should you wish to accept HTML in user input - parsing HTML input is difficult, if not impossible. Previous versions of Anti-XSS libraries included a sanitization feature for stripping "unsafe" elements from HTML input, but this approach is no longer recommended and future libraries will not include this feature. MarkDown and other text formats would be a safer option for rich input. You should never rely on validation alone. Always encode untrusted input before output, no matter what validation or sanitization you have performed.
