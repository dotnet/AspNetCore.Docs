...
1. Before putting untrusted data into an HTML element, ensure it's HTML encoded. HTML encoding takes characters such as &lt; and changes them into a safe form like &amp;lt;.
1. Before putting untrusted data into an HTML attribute, ensure that it's HTML attribute encoded. This specialized form of HTML encoding handles double quotes (\"), single quotes ('\), ampersands (\&), and less-than (\<) characters. when dealing with untrusted input,Use HTML encoding for general HTML content and HTML attribute encoding sfor HTML attributes.
